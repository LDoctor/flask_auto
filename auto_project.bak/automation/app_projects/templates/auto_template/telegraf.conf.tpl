...
[global_tags]
  ...
  # user = "$USER"
  code = "FFFFF"
  node_id = ${global_tags['node_id']}
  host_type = "physical"
  host_id = ${global_tags['host_id']}
...
[[outputs.influxdb]]
  ...
  urls = ["http://influxdb-relay.service.consul:9086"] # required
  ## The target database for metrics (telegraf will create it if not exists).
  database = "awstack" # required
  namedrop = ["bigdata_pool*"]
  # namepass = ["zongxxx"]
...
[[outputs.influxdb]]
  ...
  urls = ["http://influxdb-relay.service.consul:9086"] # required
  ## The target database for metrics (telegraf will create it if not exists).
  database = "UDPanacube" # required
  namepass = ["bigdata_pool*"]
  write_consistency = "any"
...
#  files = ["/var/log/telegraf/metrics"]
#
#   ## Data format to output.
#   ## Each data format has it's own unique set of configuration options, read
#   ## more about them here:
...
#Close plugins of libvirt
#It will cause lots of unused unix sockets to libvirtd
#[[inputs.lmem]]
#[[inputs.lcpu]]
#[[inputs.lnic]]
#[[inputs.ldisk]]
...
#     "/tmp/collect_*.sh"
#   ]
    command = "sudo /usr/local/sbin/telegraf-lxc-stats/telegraf-lxc-stats"
    data_format = "influx"
...
#   data_format = "influx"
[[inputs.exec]]
  command = "sudo /usr/bin/python /usr/local/udsafe/hpstack_telegraf.py"
  data_format = "influx"
  timeout = "10s"
  interval = "20s"
  name_prefix = 'bigdata_'
